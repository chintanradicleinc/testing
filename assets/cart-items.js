  ...
  class CartItems extends HTMLElement {
    ...
    async updateQuantity(line, quantity, name) {
      ...
      try {
        ...
        let removedProductId = lineItem.getAttribute('data-product-id');
        let allBtns = document.querySelectorAll('product-form [type="submit"][data-product_id="'+removedProductId+'"]');
        for(const btn of allBtns){
          btn.removeAttribute('data-edit');
          btn.removeAttribute('data-state');
          btn.removeAttribute('data-discipline');
          btn.removeAttribute('data-variant');
          btn.removeAttribute('data-added-variant');
        ...
        if (this.cartDrawer && newTotalQuantity === 0) {
          ...
          if (this.cartDrawer.querySelector('promoted-products')) {
            this.cartDrawer.querySelector('.drawer__content').classList.toggle('drawer__empty-with-promotions', newTotalQuantity === 0);
          }
        }
